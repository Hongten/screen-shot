...
public class CaptureView extends JWindow implements MouseListener, KeyListener,
	...
	private ScreenShotUI window;
	private ImagePanel toolPanel;
	private final int TOOLPANEL_WIDTH = 350, TOOLPANEL_HEIGHT = 300,
			HALF_PICK_IMG = 40;
	private JTextArea infoArea;
	private ToolImagePanel pickImgPanel;

	CaptureView(ScreenShotUI window, BufferedImage img) {
		super(window);
		this.window = window;
		this.desktopImg = img;
	...
	void init() {
		...
		setLayout(null);
		toolPanel = new ImagePanel(Common.SCREEN_SHOT_BACKGROUND);
		...
		infoArea.setFont(new Font(Common.REGULAR_SCRIPT, Font.PLAIN, 11));
		infoArea.setText(Common.EMPTY);
	...
	public void refreshInfoText() {
		...
		infoString = Common.X_Y + point_x + Common.COMMA + point_y + Common.W_H
				+ captureWidth + Common.COMMA + captureHeight + Common.NEW_LINE
				+ Common.RGB + point_color.getRed() + Common.COMMA
				+ point_color.getGreen() + Common.COMMA + point_color.getBlue()
				+ Common.R_PARENTHESIS + Common.NEW_LINE + text;
